 ...
 * @author kislay.verma
 */
public class OrderCancellationCashbackMIssingCategoryBuilder {
...
//        validate(requestText);
        ...
        if (orderId == null) {
            return "You request (lacking order id) has been recorded with id OrderCancellationCashbackMissingRequest" + TICKET_ID++ + ". Please provide the order id for faster resolution of the issue";
        }

        return "You request has been recorded with id OrderCancellationCashbackMissingRequest" + TICKET_ID++;
    }
